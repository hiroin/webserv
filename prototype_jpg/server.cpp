...
int http1(char *envp[])
{
  ...
  while (1) {
    ...
    if (path_string.find(".php") != std::string::npos)
    {
      ...
      send(accfd, "\r\n0\r\n", 5, 0);
      send(accfd, "\r\n", 2, 0);
    } else if (path_string.find(".jpg") != std::string::npos) {

      //ファイルサイズを取得する
      struct stat stat_buf;
      stat(path_string.c_str(), &stat_buf);

      //HTTPレスポンスを作成する
      std::string server_response;
      std::vector<std::string> header = HTTP1_Header_Jpg::make_response200(stat_buf.st_size);
      int header_size = header.size();
      for (int i = 0; i < header_size; i++) {
        server_response.append(header[i].c_str());
      }
      ...
      read(read_fd, buf, stat_buf.st_size);
      if (send(accfd, buf, stat_buf.st_size, 0) == -1) {
          std::cout << "write() failed." << std::endl;
      }
      close(read_fd);
